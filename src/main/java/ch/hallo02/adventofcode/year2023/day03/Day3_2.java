...
//     67750842
    ...
    static int check(Matcher matcher) {
        ...
        var leftCand = getAdjNumber(left);
        if (leftCand > -1) {
            matches++;
            if (c1 == -1) {
                c1 = leftCand;
        ...
        var rightCand = getAdjNumber(right);
        if (rightCand > -1) {
            matches++;
            if (c1 == -1) {
                c1 = rightCand;
        ...
        // check if top contains a symbol
        int topStart = left - 141;
        int topEnd = right - 141;
        var topCand = isADigitInRange(topStart, topEnd);
        if (topCand.size() > 0) {
            for (int y : topCand) {
                matches++;
                if (c1 == -1) {
        ...
        // check if bottom contains a symbol
        int bottomStart = left + 141;
        int bottomEnd = right + 141;
        var bottomCand = isADigitInRange(bottomStart, bottomEnd);
        if (bottomCand.size() > 0) {
            for (int y : bottomCand) {
                matches++;
                if (c1 == -1) {
                    c1 = y;
                } else if (c2 == -1) {
        ...
        return matches == 2 ? c1 * c2 : -1;
    }

    static List<Integer> isADigitInRange(int start, int end) {
        List<Integer> candidates = new LinkedList<>();

        while (start <= end) {
            if (start < 0 || start >= puzzle.length()) {
                start++;
                continue;
            }

            if (isADigit(puzzle.charAt(start))) {
                candidates.add(getAdjNumber(start));
            }
            start++;
        }
        candidates = candidates.stream().filter(v -> v != -1).toList();
    ...
    static int getAdjNumber(int pos) {
        if (Character.isDigit(puzzle.charAt(pos))) {
            String number = String.valueOf(puzzle.charAt(pos));
            int dec = pos - 1;
            while (Character.isDigit(puzzle.charAt(dec))) {
                number = puzzle.charAt(dec) + number;
                dec--;
            }
            int inc = pos + 1;
            while (Character.isDigit(puzzle.charAt(inc))) {
                number = number + puzzle.charAt(inc);
                inc++;
            }
