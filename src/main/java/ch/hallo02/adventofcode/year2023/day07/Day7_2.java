...
public class Day7_2 {
    ...
    static class Hand {
        ...
        Hand(String rawInput, Long bid) {
            ...
            this.handType = getHandType(rawInput);
            this.bid = bid;
        }
    ...
    static String stripJoker(String rawInput) {
        return rawInput.replaceAll("J", "");
    }

    static HandType getHandType(String rawInput) {

        int amountJ = amountJoker(rawInput);
        rawInput = stripJoker(rawInput);

        HandType originHandType;
        // FIVE KIND
        if (rawInput.isEmpty()) {
            originHandType = HandType.FIVEKIND;
        } else if (isFiveKind(rawInput)) {
            originHandType = HandType.FIVEKIND;
        } else if (isFourKind(rawInput)) {
            originHandType = HandType.FOURKIND;
        } else if (isFullHouse(rawInput)) {
            originHandType = HandType.FULLHOUSE;
        } else if (isThreeKind(rawInput)) {
            originHandType = HandType.THREEKIND;
        } else if (isTwoPair(rawInput)) {
            originHandType = HandType.TWOPAIR;
        } else if (isPair(rawInput)) {
            originHandType = HandType.PAIR;
        } else {
            originHandType = HandType.HIGHCARD;
        }

        return upLevelHandType(originHandType, amountJ);
    }

    static HandType upLevelHandType(HandType handType, int levels) {

        for (int i = 0; i < levels; i++) {
            handType = switch (handType) {
                case HIGHCARD -> HandType.PAIR;
                case PAIR -> HandType.THREEKIND;
                case TWOPAIR -> HandType.FULLHOUSE;
                case THREEKIND -> HandType.FOURKIND;
                case FULLHOUSE -> HandType.FOURKIND;
                case FOURKIND -> HandType.FIVEKIND;
                case FIVEKIND -> HandType.FIVEKIND;
            };
        }
        return handType;
    }
    ...
    static int amountJoker(String rawInput) {
        return (int) rawInput.chars().mapToObj(i -> Character.valueOf((char) i)).filter(c -> c == 'J').count();
    }
