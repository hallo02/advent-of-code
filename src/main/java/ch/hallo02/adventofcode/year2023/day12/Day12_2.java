...
import static ch.hallo02.adventofcode.year2023.day12.Day12_2.Spring.*;
...
public class Day12_2 {
    ...
    public static void main(String... args) {
        ...
        var r = miniPuzzle.lines()
                .map(Day12_2::lineToRow)
                .mapToLong(Day12_2::countPossibleArrangement)
                .sum();
    ...
    static long countPossibleArrangement(Row row) {
        ...
        for (int c = 0; c < Math.pow(2,numberUnknown); c++) {
            ...
            if(isValidCandidate(candidate, row)){
                ...
                newSprings.add(UNKNOWN);
                newSprings.addAll(validCandidate);
                List<Integer> newChecksum = new ArrayList<>();
                newChecksum.addAll(row.checksum);
                newChecksum.addAll(row.checksum);
                newChecksum.addAll(row.checksum);
                newChecksum.addAll(row.checksum);
                newChecksum.addAll(row.checksum);
                Row newRow = new Row(newSprings, newChecksum);

                var numberUnknown2 = 4;
                for (int c2 = 0; c2 < Math.pow(2,numberUnknown2); c2++) {
                    //System.out.println("combination bit string " + combinationBitString.get(c));
                    //patch
                    List<Spring> candidate2 = getCandidates(newRow, numberUnknown2, c2);
                    if (isValidCandidate(candidate2, newRow)){
        ...
        System.out.println("Line r " + possibleArrangements);
        return  possibleArrangements;
    }

    static List<Spring> getCandidates(Row row, long numberUnknown, int c){
        ...
        for (int s = 0; s < row.springs.size(); s++) {
            ...
            if (spring == UNKNOWN) {
                candidate.add(String.format("%" + numberUnknown + "s", Integer.toBinaryString(c)).replace(' ', '0').charAt(numberUnknownInRow) == '0' ? DEFECT : OPERATIONAL);
                numberUnknownInRow++;
            } else {
    ...
    static long getNumberUnknowns(Row row){
        return row.springs
                .stream()
                .filter(e -> e == UNKNOWN)
                .count();
    }
    static boolean isValidCandidate(List<Spring> candidate, Row row){
