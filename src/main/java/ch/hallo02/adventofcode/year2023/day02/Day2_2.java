...
public class Day2_2 {
    ...
    public static void main(String[] args) {
        ...
        var result = puzzle.lines()
                .map(l -> getGame(l))
                .mapToInt(game -> getPower(game))
                .sum();
        System.out.println(result);
    }

    static int getPower(Game game){
        int red = 0;
        int green = 0;
        int blue = 0;

        for(Expose e: game.exposes){
            red = e.red > red ? e.red : red;
            green = e.green > green ? e.green : green;
            blue = e.blue > blue ? e.blue : blue;
        };
    ...
    static Game getGame(String line) {
        int gameId = Integer.valueOf(line.split(":")[0].split(" ")[1]);

        String[] exposesString = line
                .split(":")[1]
                .split(";");

        Expose[] exposes = new Expose[exposesString.length];
        for (int i = 0; i < exposes.length; i++) {
            exposes[i] = getExpose(exposesString[i]);
        }

        Game g = new Game(gameId, exposes);
        System.out.println(g);
        return g;

    }

    static Expose getExpose(String partialLine) {
        String[] colors = partialLine.trim().split(",");
        int red = 0;
        int blue = 0;
        int green = 0;
        for (String color : colors) {
            if (color.contains("red")) {
                red = Integer.valueOf(color.trim().split(" ")[0]);
            } else if (color.contains("blue")) {
                blue = Integer.valueOf(color.trim().split(" ")[0]);
            } else {
                green = Integer.valueOf(color.trim().split(" ")[0]);
            }
        }
        return new Expose(red, blue, green);
    }
    ...
    static class Expose {
        ...
        Expose(int red, int blue, int green) {
            this.red = red;
            this.blue = blue;
            this.green = green;
        }
        ...
        public String toString() {
            return String.format("[red:%s,blue:%s,green:%s]", red, blue, green);
        }
    }
