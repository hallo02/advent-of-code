package ch.hallo02.adventofcode.year2023.day10;
...
public class Day10_2 {
    ...
    public static void main(String... args) {
        ...
        List<List<Direction>> fieldTemp = puzzle.lines()
                .map(Day10_2::lineToDirections)
                .toList();

        var field = fieldToArray(fieldTemp);
        LinkedList<Position> path = new LinkedList<>();

        var s = findS(field);
        path.add(s);

        var next = findOnePossibleStepAfterS(field, s);
        path.add(next);
        move(field, path);
        printPath(path);


        var result = getAmountOfNestsInsideCircle(field, path);
        System.out.println("Result: " + result);


    }

    public static int getAmountOfNestsInsideCircle(Direction[][] field, List<Position> path) {

        int matches = 0;

        for (int x = 0; x < field.length; x++) {
            for (int y = 0; y < field[0].length; y++) {
                var candidate = new Position(x, y, null);
                ...
                while (xCheck >= 0 && yCheck >= 0) {
                    var foundInPath = path.indexOf(new Position(xCheck, yCheck, null));
                    if (foundInPath > -1 && checkEdgeCase(foundInPath, path)) {
                        c++;
                    }
                    xCheck--;
                    yCheck--;
    ...
    public static boolean checkEdgeCase(int foundInPath, List<Position> path) {
        // Current
        int x = path.get(foundInPath).x;
        int y = path.get(foundInPath).y;

        // Before
        int positionBefore = foundInPath == 0 ? path.size() - 1 : foundInPath - 1;
        int xBefore = path.get(positionBefore).x;
        int yBefore = path.get(positionBefore).y;

        // After
        int positionAfter = foundInPath == path.size() - 1 ? 0 : foundInPath + 1;
        int xAfter = path.get(positionAfter).x;
        int yAfter = path.get(positionAfter).y;

        if(xAfter >= x && xBefore >= x && yAfter <= y && yBefore <= y){
            return false;
        }
        if(xAfter <= x && xBefore <= x && yAfter >= y && yBefore >= y){
            return false;
    ...
    public static List<Integer> getSortedPathYList(LinkedList<Position> path) {
        return path.stream().mapToInt(e -> e.y).sorted().boxed().toList();
    }

    public static List<Integer> getSortedPathXList(LinkedList<Position> path) {
        return path.stream().mapToInt(e -> e.x).sorted().boxed().toList();
    }

    static void printPath(List<Position> paths) {
        paths.forEach(System.out::println);
    }

    static void move(Direction[][] field, List<Position> path) {

        Position current = path.get(path.size() - 1);
        while (!current.equals(path.get(0))) {
            current = switch (current.direction) {
                case VERTICAL -> getOneOfTwoPossibleNewPositions(path,
                        new Position(current.x, current.y - 1, field[current.x][current.y - 1]),
                        new Position(current.x, current.y + 1, field[current.x][current.y + 1]));

                case HORIZONTAL -> getOneOfTwoPossibleNewPositions(path,
                        new Position(current.x - 1, current.y, field[current.x - 1][current.y]),
                        new Position(current.x + 1, current.y, field[current.x + 1][current.y]));
                case TOPRIGHT -> getOneOfTwoPossibleNewPositions(path,
                        new Position(current.x + 1, current.y, field[current.x + 1][current.y]),
                        new Position(current.x, current.y - 1, field[current.x][current.y - 1])
                );
                case TOPLEFT -> getOneOfTwoPossibleNewPositions(path,
                        new Position(current.x, current.y - 1, field[current.x][current.y - 1]),
                        new Position(current.x - 1, current.y, field[current.x - 1][current.y]));
                case BOTTOMLEFT -> getOneOfTwoPossibleNewPositions(path,
                        new Position(current.x - 1, current.y, field[current.x - 1][current.y]),
                        new Position(current.x, current.y + 1, field[current.x][current.y + 1]));
                case BOTTOMRIGHT -> getOneOfTwoPossibleNewPositions(path,
                        new Position(current.x + 1, current.y, field[current.x + 1][current.y]),
                        new Position(current.x, current.y + 1, field[current.x][current.y + 1]));
    ...
    static Position getOneOfTwoPossibleNewPositions(List<Position> path, Position pos1, Position pos2) {
        return path.contains(pos2) ? pos1 : pos2;
    }

    enum Direction {
        VERTICAL('|'), HORIZONTAL('-'), TOPRIGHT('L'), TOPLEFT('J'), BOTTOMLEFT('7'), BOTTOMRIGHT('F'), GROUND('.'), STARTING('S');
        ...
        public static Direction getDirectionBySign(char sign) {
    ...
    static Direction[][] fieldToArray(List<List<Direction>> fieldTemp) {
        // x y
        Direction[][] array = new Direction[fieldTemp.get(0).size()][fieldTemp.size()];

        for (int y = 0; y < fieldTemp.size(); y++) {
            for (int x = 0; x < fieldTemp.get(0).size(); x++) {
                array[x][y] = fieldTemp.get(y).get(x);
            }
        }
        return array;
    }
    ...
    static Position findOnePossibleStepAfterS(Direction[][] field, Position starting) {
        // HACK, because I know the data
        Direction above = field[starting.x][starting.y - 1];
        if (above == Direction.VERTICAL || above == Direction.BOTTOMLEFT || above == Direction.BOTTOMRIGHT) {
            return new Position(starting.x, starting.y - 1, above);
        }
        Direction right = field[starting.x + 1][starting.y];
        if (right == Direction.HORIZONTAL || right == Direction.TOPLEFT || right == Direction.BOTTOMLEFT) {
            return new Position(starting.x + 1, starting.y, right);
        }
        throw new RuntimeException("not found a possible step after");

    }

    static Position findS(Direction[][] field) {
        for (int x = 0; x < field.length; x++) {
            for (int y = 0; y < field[0].length; y++) {
                if (field[x][y] == Direction.STARTING) {
                    return new Position(x, y, Direction.STARTING);
                }
            }
        }
        throw new RuntimeException("No stating point found");
    }
    ...
    static List<Direction> lineToDirections(String l) {
        return l.chars().mapToObj(c -> Direction.getDirectionBySign((char) c)).toList();
    }
