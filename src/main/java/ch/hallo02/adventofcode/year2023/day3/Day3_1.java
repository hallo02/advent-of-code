...
public class Day3_1 {
    ...
    static boolean check(Matcher matcher) {
        // check if left is a symbol
        int left = matcher.start() - 1;

        if (isASymbolInRange(left, left, ((left + 1) / 141))) {
        ...
        if (isASymbolInRange(right, right, ((left + 1) / 141))) {
        ...
        if (isASymbolInRange(topStart, topEnd, ((left + 1) / 141) - 1)) {
        ...
        if (isASymbolInRange(bottomStart, bottomEnd, ((left + 1) / 141) + 1)) {
    ...
    static boolean isASymbolInRange(int start, int end, int expLine) {
        while (start <= end) {
            if (start < 0 || start >= puzzle.length() || (start / 141) != expLine) {
                start++;
                continue;
            }
            if (isASymbol(puzzle.charAt(start))) {
                return true;
            }
            start++;
        }
    ...
    static boolean isASymbol(char c) {
        var r = !(Character.isDigit(c) || c == '.' || c == '\n');
        if(r){
