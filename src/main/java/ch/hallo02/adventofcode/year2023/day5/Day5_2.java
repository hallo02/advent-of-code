...
public class Day5_2 {
    ...
    public static void main(String... args) {
        ...
        List<Triple> seedToSoil = new LinkedList<>(getSeedToSoil());
        List<Triple> soilToFert = new LinkedList<>(getSoilToFert());
        List<Triple> fertToWater = new LinkedList<>(getFertToWater());
        List<Triple> waterToLight = new LinkedList<>(getWaterToLight());
        List<Triple> lightToTemp = new LinkedList<>(getLightToTemperature());
        List<Triple> tempToHumidity = new LinkedList<>(getTemperatureToHumidity());
        List<Triple> humidityToLocation = new LinkedList<>(getHumidityToLocation());

        seedToSoil.sort(Comparator.comparingLong(t -> t.srcStart));
        soilToFert.sort(Comparator.comparingLong(t -> t.srcStart));
        fertToWater.sort(Comparator.comparingLong(t -> t.srcStart));
        waterToLight.sort(Comparator.comparingLong(t -> t.srcStart));
        lightToTemp.sort(Comparator.comparingLong(t -> t.srcStart));
        tempToHumidity.sort(Comparator.comparingLong(t -> t.srcStart));
        humidityToLocation.sort(Comparator.comparingLong(t -> t.srcStart));

        List<Long> mins = new LinkedList<>();

        var i = 1;
        var seedsIt = seeds.iterator();
        while (seedsIt.hasNext()) {
            var init = seedsIt.next();
            var range = seedsIt.next();
            System.out.println("range " + i++);
            mins.add(LongStream.range(init, init + range)
                    .map(n -> access(seedToSoil, n))
                    .map(n -> access(soilToFert, n))
                    .map(n -> access(fertToWater, n))
                    .map(n -> access(waterToLight, n))
                    .map(n -> access(lightToTemp, n))
                    .map(n -> access(tempToHumidity, n))
                    .map(n -> access(humidityToLocation, n))
                    .min().orElseThrow());
        }


        System.out.println(mins.stream().mapToLong(s -> s).min().orElseThrow());

    }


    static Long access(List<Triple> triple, Long searchFor) {

        var tripleIterator = triple.iterator();
        Triple last = null;
        Triple current = null;
        Triple candidate = null;

        while (tripleIterator.hasNext()) {
            last = current;
            current = tripleIterator.next();

            if (current.srcStart > searchFor) {
                if (last == null) {
                    return searchFor;
                }
                candidate = last;
...
//        10 + 12 - 10 = 12
        ...
        if (candidate.srcStart <= searchFor && searchFor < candidate.srcStart + candidate.length) {
            var r = candidate.destStart + (searchFor - candidate.srcStart);
            return r;
        }
    ...
    static List<Long> getSeeds() {
        String puzzle = "seeds: 3943078016 158366385 481035699 103909769 3553279107 15651230 3322093486 189601966 2957349913 359478652 924423181 691197498 2578953067 27362630 124747783 108079254 1992340665 437203822 2681092979 110901631";
        return Arrays.stream(puzzle.split(":")[1].split(" "))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
    ...
    static List<Triple> getSeedToSoil() {
        String puzzle = """
                    seed-to-soil map:
        ...
        return puzzle.lines()
                ...
                .map(l -> {
                    ...
                    return new Triple(splits[0], splits[1], splits[2]);
                })
                .toList();
    }

    static List<Triple> getSoilToFert() {
        String puzzle = """
    ...
    static List<Triple> getFertToWater() {
        String puzzle = """
    ...
    static List<Triple> getWaterToLight() {
        String puzzle = """
                      water-to-light map:
    ...
    static List<Triple> getLightToTemperature() {
        String puzzle = """
    ...
    static List<Triple> getTemperatureToHumidity() {
        String puzzle = """
    ...
    static List<Triple> getHumidityToLocation() {
        String puzzle = """
    ...
    static class Triple {
        ...
        public Triple(String destStart, String srcStart, String length) {
            this.destStart = Long.valueOf(destStart.trim());
            this.srcStart = Long.valueOf(srcStart.trim());
            this.length = Long.valueOf(length.trim());
        }

        public Triple(int destStart, int srcStart, int length) {
        ...
        public String toString() {
            return String.format("from %s, to %s, length %s", srcStart, destStart, length);
        }
    }
}
